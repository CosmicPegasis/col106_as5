...
TEST(syncSymTable, BasicTest){
  ExprTreeNode* root = parse_token(":=");
  ExprTreeNode* left = parse_token("v");
  ExprTreeNode* right = parse_token("5");
  root->left = left;
  root->right = right;
  SymbolTable s;
  int address = syncSymtable(root, &s);
  ASSERT_EQ(s.get_root()->address, -1);
  ASSERT_EQ(address, -1);
}
TEST(syncSymTable, DeleteTest){
  ExprTreeNode* root = parse_token(":=");
  ExprTreeNode* left = parse_token("v");
  ExprTreeNode* right = parse_token("5");
  root->left = left;
  ...
  syncSymtable(root, &s);
  root = parse_token(":=");
  root->left = parse_token("del");
  root->right = parse_token("v");
  s.assign_address("v", 0);
  int address = syncSymtable(root, &s);
  ASSERT_EQ(s.get_root(), nullptr);
  ASSERT_EQ(address, 0);
}
TEST(syncSymTable, DeleteNonExistent){
  ExprTreeNode* root = parse_token(":=");
  ExprTreeNode* left = parse_token("u");
  ExprTreeNode* right = parse_token("5");
  root->left = left;
  root->right = right;
  SymbolTable s;
